"""
# Note: requires scikit-learn==1.9.0 to match model training version
...
def predict_pressure_curve(fac_row: pd.Series, is_weekend: int) -> dict[int, float]:
    ...
    if ml_model is not None and ml_features:
        ...
        for h in range(6, 23):
            ...
            # Model features: facility_name, hour_sin, hour_cos, day_sin, day_cos,
            # is_weekend, is_peak_hour, previous_occupancy, occupancy_rolling_3, base_rate_kes
            facility_name = str(fac_row.get("facility_name_clean", fac_row.name if hasattr(fac_row, 'name') else "Unknown"))
            row_dict = {
                ...
                "previous_occupancy": base / 100.0,  # model trained on 0-1 scale
                "occupancy_rolling_3": base / 100.0,
            ...
            try:
                pred_raw = float(ml_model.predict(X)[0])
                # Model outputs pressure score 0-100
                pred = float(np.clip(pred_raw, 0, 100))
            except Exception:
