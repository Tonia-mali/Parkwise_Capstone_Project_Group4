...
Each run:
  ...
  2. Pulls one live traffic reading per facility in the sample.
...
Windows (Task Scheduler):
  ...
  2. Trigger: Daily, then set it to repeat every 2-3 hours during the hours you
...
macOS/Linux (cron):
  Run `crontab -e` and add a line like:
    0 6-22/2 * * * cd /path/to/project && /path/to/python collect_traffic_snapshot.py >> snapshot_log.txt 2>&1
  which runs every 2 hours between 6 AM and 10 PM.
...
FACILITY_SAMPLE_PATH = "nairobi_parking_traffic_signal.csv"
N_SAMPLE_FACILITIES = 40
...
def collect_snapshot():
    ...
    try:
        facilities = pd.read_csv(FACILITY_SAMPLE_PATH).head(N_SAMPLE_FACILITIES)
    except FileNotFoundError:
        print(f"Could not find '{FACILITY_SAMPLE_PATH}'. Run the notebook pipeline through "
              "the traffic signal step at least once before scheduling this script.")
        return None
